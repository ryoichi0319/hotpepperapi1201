...
export const authRouter = router({
    ...
    .mutation(async({ input }) =>{
        ...
        //パスワード変更
        updatePassword: privateProcedure.input(
            z.object({
            ...
            try{
                ...
                if(isSamePassword){
                    throw new TRPCError({
                        code: "BAD_REQUEST",
                        message: "現在のパスワードと新しいパスワードが同じです。別のパスワードを設定してください"
                    })
                }
            ...
            try{
                ...
                if(existingToken){
                    throw new TRPCError({
                        code: "BAD_REQUEST",
                        message: 
                        "既にパスワード再設定用のメールをお送りしました。１時間後に再度お試しください"
                    })
                }
            ...
            try{
                ...
                if(!foundToken){
                    throw new TRPCError({
                        code: "BAD_REQUEST",
                        message:
                        "無効なトークンです。再度パスワードを再設定を行なってください"
                    })
                }
                //現在の日時
                const now = new Date()
                ...
                if(now > foundToken.expiry){
                    throw new TRPCError({
                        code: "BAD_REQUEST",
                        message: "トークンの期限が切れています。再度パスーワード再設定を行なってください"
                    })
                }
